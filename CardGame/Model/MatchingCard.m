...
@implementation MatchingCard

- (NSString *)contents
{
    return [[MatchingCard rankStrings][self.rank] stringByAppendingString:self.suit];
}
...
{
    if ([[MatchingCard validSuit] containsObject:suit])
        _suit = suit;
}
...
- (int)match:(NSArray *)otherCards
{
    ...
    if ([otherCards count] == 1) {
        MatchingCard *otherCard = [otherCards lastObject];
        if ([otherCard.suit isEqualToString:self.suit])
            score = 2;
        else if (otherCard.rank == self.rank)
            score = 6;
    } else if ([otherCards count] == 2) {
        MatchingCard *otherCard1 = otherCards[0];
        MatchingCard *otherCard2 = otherCards[1];
